#!/usr/bin/env bash
# Transport E2E Test Runner (bd-26l3)
#
# Runs the transport layer E2E tests (TCP connect/echo/close, UDP send/recv,
# large transfers) with deterministic settings and artifact capture.
#
# Usage:
#   ./scripts/test_transport_e2e.sh [test_filter]
#
...
OUTPUT_DIR="${PROJECT_ROOT}/target/e2e-results/transport"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
RUN_STARTED_TS="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
LOG_FILE="${OUTPUT_DIR}/transport_e2e_${TIMESTAMP}.log"
...
echo "==================================================================="
echo "              Asupersync Transport E2E Tests                       "
echo "==================================================================="
echo ""
echo "Config:"
...
if ! cargo check --test e2e_transport --all-features 2>"${ARTIFACT_DIR}/compile_errors.log"; then
...
# --- Section: Run Tests ---
echo ""
echo ">>> [2/4] Running transport E2E tests..."

TEST_RESULT=0
CARGO_ARGS=(--test e2e_transport --all-features)
...
check_pattern "connection refused"  "connection refused"
check_pattern "address already in use" "port collision"
...
SUITE_ID="transport_e2e"
SCENARIO_ID="E2E-SUITE-TRANSPORT"
...
grep -oE "local_addr[= ]+[0-9.:]+" "$LOG_FILE" > "${ARTIFACT_DIR}/endpoints.txt" 2>/dev/null || true
