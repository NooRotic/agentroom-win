...
RUN_ROOT="${1:-/tmp/doctor_frankentui/e2e/failure_${TIMESTAMP_UTC}}"
...
{
  echo "doctor_frankentui_failure_e2e"
...
finalize_failure() {
  python3 - \
    ...
    "${EVENT_VALIDATION_REPORT_JSON}" \
    "${LOG_DIR}" \
    "${ROOT_DIR}/scripts/doctor_frankentui_validate_jsonl.py" \
    "${ENV_SNAPSHOT}" <<'PY'
...
if not rows:
    bootstrap_stdout = log_dir / "build_doctor.stdout.log"
    bootstrap_stderr = log_dir / "build_doctor.stderr.log"
    bootstrap_vhs_stdout = log_dir / "vhs_smoke.stdout.log"
    bootstrap_vhs_stderr = log_dir / "vhs_smoke.stderr.log"
    bootstrap_vhs_tape = run_root / "meta" / "vhs_smoke" / "smoke.tape"
    bootstrap_artifacts = [
        bootstrap_stdout,
        bootstrap_stderr,
        bootstrap_vhs_stdout,
        bootstrap_vhs_stderr,
        bootstrap_vhs_tape,
        global_env_snapshot,
    ]
    ...
    for artifact in bootstrap_artifacts:
        if artifact.exists():
            bootstrap_hashes[str(artifact)] = sha256_file(artifact)
        else:
            bootstrap_missing.append(str(artifact))

    rows.append(
        {
            "case_id": "bootstrap_build",
            "expected_exit": 0,
            "actual_exit": 1,
            "pass": False,
            "regex_matched": False,
            "expected_regex": "bootstrap must succeed",
            "expect_json": False,
            "json_valid": False,
            "duration_seconds": 0,
            "stdout_log": str(bootstrap_stdout),
            "stderr_log": str(bootstrap_stderr),
            "env_snapshot": str(global_env_snapshot),
            "key_artifact_paths": [str(path) for path in bootstrap_artifacts],
            "artifact_hashes": bootstrap_hashes,
            "missing_artifacts": bootstrap_missing,
        }
    )
...
events.append(
    {
        ...
        "command": "doctor_frankentui_failure_e2e",
        "env_hash": env_hash,
        "duration_ms": 0,
        "exit_code": 0,
        "stdout_sha256": None,
...
vhs_smoke_preflight() {
  ...
  local stdout_log="${LOG_DIR}/vhs_smoke.stdout.log"
  local stderr_log="${LOG_DIR}/vhs_smoke.stderr.log"
...
EOF
  ...
  printf '[vhs_smoke] ' >> "${COMMAND_MANIFEST}"
  printf '%q ' python3 - "${tape_path}" >> "${COMMAND_MANIFEST}"
  printf '\n' >> "${COMMAND_MANIFEST}"

  set +e
  python3 - "${tape_path}" > "${stdout_log}" 2> "${stderr_log}" <<'PY'
from __future__ import annotations
...
PY
  ...
  if [[ "${exit_code}" -ne 0 ]]; then
    echo "[e2e-failure] vhs smoke check failed (exit=${exit_code})" >&2
    echo "[e2e-failure] stderr log: ${stderr_log}" >&2
    exit "${exit_code}"
  fi
}
...
run_build() {
  ...
  printf '[build_doctor] cargo build -p doctor_frankentui\n' >> "${COMMAND_MANIFEST}"
  cargo build -p doctor_frankentui > "${stdout_log}" 2> "${stderr_log}"
...
prepare_seed_retry_wrapper() {
  ...
  # doctor_frankentui:no-fake-allow (e2e) generates a local Python HTTP server script to
  # exercise seed-demo retry/timeout semantics using real sockets (no fake binary shims).
...
PY
  ...
  # doctor_frankentui:no-fake-allow (e2e) generates a local runner script that starts the
  # server and invokes seed-demo against it (real subprocess orchestration).
...
echo "[e2e-failure] completed doctor_frankentui failure matrix"
