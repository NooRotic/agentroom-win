...
pub struct WatcherState {
    ...
    known_jsonl_files: HashMap<PathBuf, String>, // path → agent_id (derived from filename)
}
...
fn scan_and_process(
    ...
    for entry in entries.flatten() {
        ...
        if !state.known_jsonl_files.contains_key(&path) {
            ...
            state
                .known_jsonl_files
                .insert(path.clone(), agent_id.clone());
        ...
        let agent_id = state.known_jsonl_files.get(&path).unwrap().clone();
        let had_tools = state
            .state_manager
            .agents
            .get(&agent_id)
        ...
        for line in lines {
            ...
            state.state_manager.process_events(&agent_id, events);
        }
    }
}
...
fn process_file_event(
    ...
    if !state.known_jsonl_files.contains_key(path) {
        ...
        state
            .known_jsonl_files
            .insert(path.to_path_buf(), agent_id.clone());
        state.file_states.insert(
            path.to_path_buf(),
            FileReadState {
                offset: 0,
